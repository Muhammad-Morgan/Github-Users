...
That’s all Tailwind needs in v4 (no @tailwind base / components / utilities anymore).

first add @tailwindcss/vite to the devDependency

```bash
npm install -D @tailwindcss/vite
```

Wiring Tailwind into Vite
vite.config.ts
...
const SearchForm = ({ userName, setUserName }: SearchFormProps) => {
  ...
  return (
    <form onSubmit={handleSearch} className="mb-8 flex w-full items-center gap-x-2 lg:w-1/3">
...
const UsedLanguages = ({ repositories }: { repositories: Repository[] }) => {
  ...
  return (
    <div>
      <h2 className="mb-4 text-center text-2xl font-semibold">Used Languages</h2>
